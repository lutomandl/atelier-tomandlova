    ...
    {#each images as image}
      <div>
        <div
          ...
          out:send={{ key: `${STRAPI_BASE_URL}${image.formats.small.url}` }}
          in:receive={{ key: `${STRAPI_BASE_URL}${image.formats.small.url}` }}
          on:click={() => handlePreviewClick(`${STRAPI_BASE_URL}${image.url}`)}
          class="image"
          style="background-image: url({`${STRAPI_BASE_URL}${image.formats.small.url}`});"
        />
      </div>
    {/each}
  </div>
    ...
    >
      ...
      >
        {#each images as image}
          <div
            role="img"
            data-selected={selected === `${STRAPI_BASE_URL}${image.url}`}
            class:active={selected === `${STRAPI_BASE_URL}${image.url}`}
            on:click={() => (selected = `${STRAPI_BASE_URL}${image.url}`)}
            class="image"
            style="background-image:url({`${STRAPI_BASE_URL}${image.url}`})"
          />
        {/each}
      </div>
    </div>
